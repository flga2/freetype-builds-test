...
test-ft:
	CGO_ENABLED=1 GOOS=darwin GOARCH=386 go build -tags 'static' -ldflags "-linkmode external -extldflags -static" -o static main.go
	./static $(version)
test-ft-hb:
	CGO_ENABLED=1 GOOS=darwin GOARCH=386 go build -tags 'static harfbuzz' -ldflags "-linkmode external -extldflags -static" -o statichb main.go
	./statichb $(version)